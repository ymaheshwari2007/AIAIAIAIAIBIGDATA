...
- Stage 4 — 🔨 **in progress**: the triage agent — paste a repo URL → OSV-SCALIBR extracts deps → deterministic match (version-in-range) → LLM judges real impact → structured findings + per-project memory. Provider-agnostic + BYO-key, Gemini free-tier default. Sub-stages 4a–4d. Done when pointing it at a repo gives a report that makes sense.
...
- **Untrusted repos:** each scan runs in a **sandboxed, ephemeral, egress-restricted Fargate task** — clone + `scalibr` only, **never execute repo code**, no secrets mounted.
- **Multi-tenancy:** auth (Cognito or similar), per-user data isolation, rate limits/quotas, **BYO-key** so users fund their own LLM calls.
